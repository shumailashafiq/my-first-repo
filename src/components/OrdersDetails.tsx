...
const OrdersDetails = (props) => {
  ...
  const getShippingAddress = () => {
    ...
    axios
      .get(
        baseURL + 'customerAddress/findById/' + OrderIndex.customer_Address_id,
      )
      .then((res) => {
        console.log(res.data.object);
        setShippingAddress(res.data.object);
  ...
  const getBillingAddress = () => {
    ...
    axios
      .get(baseURL + 'billingAddress/findById/' + OrderIndex.billing_address_id)
      .then((res) => {
        setShippingAddress(res.data.object);
      })
      .catch((err) => {
    ...
    <div className="rounded-sm border relative border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="max-w-full overflow-x-auto pt-4 px-8">
        ...
        <div className=" flex w-full justify-between p-8 ">
          <div>Costomer ID : {OrderIndex.customer_id}</div>
          <div className="pr-5">Total Amount : ${OrderIndex.total_amount}</div>
        </div>
        <hr />
        <div className=" flex w-full justify-between p-8 ">
          <div
            onClick={getShippingAddress}
            className={` pr-5 inline-flex items-center cursor-pointer justify-center gap-2.5 py-4 px-10 text-center font-medium border-2 border-meta-3  ${
              activeAddress === 'Shipping'
                ? 'bg-meta-3 text-white'
                : 'bg-white text-meta-3'
          ...
          <div
            ...
            className={` pr-5 inline-flex items-center cursor-pointer justify-center gap-2.5 py-4 px-10 text-center font-medium border-2 border-meta-3 ${
              activeAddress === 'Billing'
                ? 'bg-meta-3 text-white'
                : 'bg-white text-meta-3'
            } hover:text-opacity-90 lg:px-8 xl:px-10`}
          >
            Billing Address
          </div>
        </div>

        {ShippingAddress.length === 0 ? null : (
          <div
            className={` ${
              activeAddress !== null ? 'flex' : 'hidden'
            } flex-col w-full justify-between px-8 gap-4  mb-5`}
          >
            <h3 className="font-bold text-black text-xl">
              {activeAddress} Address &#40;{ShippingAddress.addressType}&#41;
            </h3>
            <p>
            ...
            <p>
              ...
              {ShippingAddress?.cityId?.cityName}, &#40;
              {ShippingAddress?.cityId?.country?.countryName}&#41;
            </p>
          </div>
        )}
      </div>
